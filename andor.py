...
class Controller:
    ...
    def _set_shutter(self, shutter, ttl=1):
        ...
        shutter_list = []
        shutter_list.append(["OutputTTL", 1])
        if shutter in self.shutter_dict:
            shutter_list.append(
                ['ShutterTimingMode',
                 self.shutter_dict[shutter]]
            )
            shutter_list.append(["ShutterClosingDelay", 20])  # Delays are in milliseconds
            shutter_list.append(["ShutterOpeningDelay", 20])
            return shutter_list
        else:
    ...
    def initialize(self, path_to_lib="", wait_to_cool=True):
        ...
        logger.info("Loading Andor SDK libaray")
        try:
            self.opt = Andor()
        ...
        connected_cams = self.opt.GetAvailableCameras()[1] - 1
        camera_list = []
        for cams in range(connected_cams):
            camera_list.append(self.opt.GetCameraHandle(cams)[1])
